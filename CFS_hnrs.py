    ...
    for f in range(1):  # change the Gaussian kernel parameter
        ...
        for g in range(1):
            for l in range(1):
                ...
                original_select, original_remian = HSNRS.get_attribute_reduction(all_data, all_label, para_s)
                ...
                init_hypersphere = HSNRS.HyperSphere(init_data, init_label, len(init_class_label), init_class_label,
                                                     para_s).init_hyper_sphere()
                select_att, remain_att = HSNRS.get_attribute_reduction(init_data, init_label, para_s)
                print("init select_att, remain_att:", select_att, remain_att)
                ...
                for i in range(len(new_data_list)):
                    ...
                    new_hypersphere, cluster_data, cluster_label, cluster_label_list = open_learning.BallUpdate(
                        unknown_data).new_balls("dbscan", para_s, i)
                    ball_update = open_learning.BallUpdate(unknown_data).knowledge_update(init_hypersphere,
                                                                                          new_hypersphere)
                    ...
                    if len(remain_att) == 0:
                        ...
                        select_att, remain_att = open_learning.SelectFeature(select_att, remain_att,
                                                                             ball_update).select_feature(
                            temp_data, temp_label, para_s)
                        print("new turn select_att, remain_att:", select_att, remain_att)
                ...
                print("new_select_att, remain_att, end_time", select_att, remain_att, end_time)
