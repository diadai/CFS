...
class HyperSphere:
    ...
    def __init__(self, data, label, class_num, class_list, para_s):
        self.data = data
        self.label = label
        self.s = para_s
    ...
    def get_radius_and_centers(self, fd_k):
        ...
        term_4 = np.mean(-2 * np.dot(cur_sim_vec, fd_k.alpha))
        #center
    ...
    def get_distance_to_center(self, att, parameter):
        ...
        for i in range(self.class_num):
            ...
            non_sv_index = list(set(data_index) - set(parameter["sv_index"][i]))
...
def get_attribute_reduction(data, label, para_s):
    ...
    hyper_sphere = HyperSphere(data, label, class_num, class_list, para_s)
    ...
    for i in range(data.shape[1]):
        ...
        if num_mis == 0:
            hyper_sphere = HyperSphere(data[:, attributes_reduction], label, class_num, class_list, para_s)
            parameters = hyper_sphere.init_hyper_sphere()
        else:
